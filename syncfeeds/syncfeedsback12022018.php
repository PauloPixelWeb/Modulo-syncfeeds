                    ...
                    if (!Db::getInstance()->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'syncfeeds_order` WHERE `id_order`=' . $pedido['id_order'] . ' AND `id_state`=' . $estado_valor))
                    {
                        ...
                        foreach ($detalles_pedido as $detalle)
                        {
                            ...
                            $xml .= '<OrderItem>
                                    ...
                                    <TotalAmount>' . round((float) ($detalle['product_quantity'] * $detalle['product_price'])) . '</TotalAmount>
                                    <TotalDiscountAmount>' . round((float) ($detalle['product_quantity'] * $detalle['product_price'] - $detalle['total_price_tax_excl'])) . '</TotalDiscountAmount>
                                    <TotalNetAmount>' . round((float) $detalle['total_price_tax_excl']) . '</TotalNetAmount>
                                </OrderItem>';
                        }
